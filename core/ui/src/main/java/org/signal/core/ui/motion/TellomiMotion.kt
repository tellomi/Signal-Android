 ...
 *
 * 页面代码不写时长和曲线，只从这里取。数值直接取自 Material 3 MotionScheme 的官方 token：
...
object TellomiMotion {
  ...
  /** 菜单、Sheet 落档、共享元素、列表补位：Expressive 空间默认。 */
  val Move = SpringToken(stiffness = 380f, dampingRatio = 0.8f)

  /** 整屏级（查看器开合）：Expressive 空间慢。 */
  ...
  /**
   * 动效档位（标准第七节）：
   * - [FULL]：全部动效，允许实时模糊（API 31+）。
   * - [SIMPLIFIED]：关实时模糊、循环 / 背景动画、飞行类动画（例如回应飞入），共享元素改淡入淡出；弹簧照常。
  ...
  fun tierFor(signals: DeviceSignals): Tier {
    return when {
      ...
      signals.sdkInt < Build.VERSION_CODES.S -> Tier.SIMPLIFIED
      else -> Tier.FULL
    }
  }
  ...
  fun allowsLiveBlur(tier: Tier): Boolean = tier == Tier.FULL
